...
public class BackTest2 {
        ...
        while (!over) {
            ...
            for (MarketType market : marketTypeList) {
                List<FiveMinutesCandle> fiveMinutesCandles = fiveMinutesCandleRepository.findFiveMinutesCandlesLimitOffset(
                        market.getType(), LocalDateTime.of(2017, 12, 1, 0, 0, 0), limit, offset);
                ...
                AccountCoinWallet wallet = fetchWallet(market, baseCandle);
                ...
                boolean isAsk = isAskable && 매도신호(AskStrategy.STRATEGY_4, bollingerBands, rsi14, candles);
                ...
                boolean isBid = isBidable && 매수신호(BidStrategy.STRATEGY_4, bollingerBands, rsi14, candles);
                if (isBid) { // 매수
                    log.info("{} 현재 캔들", targetCandle.getCandleDateTimeKst());
                    매수(targetCandle, wallet);
                    log.info("{}% \r\n", targetCandle.getCandlePercent());
                }
                printWalletBalance(market);
    ...
    private void printWalletBalance(MarketType market) {
        AccountCoinWallet wallet = accountCoinWalletRepository.findByMarket(market);
        if (!wallet.isEmpty()) {
            log.info("[{}] 평단가 : {}, 수익률 : {}%, 수익금 : {}, KRW : {}원, 평가금액 : {}"
    ...
    private void 매도(FiveMinutesCandle targetCandle, AccountCoinWallet wallet) {
        ...
        // 매도 -> 다음 캔들 시가에 매도
        backTestOrdersRepository.save(BackTestOrders.of(wallet.getMarket(), ASK, targetCandle.getOpeningPrice(), wallet.getVolume(), wallet.getValAmount() * 0.0005, targetCandle.getTimestamp(), wallet.getProceeds(), wallet.getProceedRate()));
    ...
    private boolean 매도신호(AskStrategy askStrategy, BollingerBands bollingerBands, RSIs rsi14, List<FiveMinutesCandle> candles) {
        switch (askStrategy) {
